...
# Create reduced version of data frames with counts of at least [update me to be different for each of the taxa]
min <- 50
...
(Xwithin.order <-   chisq.test(table(Xdata_order$Host_order, Xdata_order$any_coinf_within)))
(Xwithin.family <-  chisq.test(table(Xdata_family$Host_family, Xdata_family$any_coinf_within)))
(Xwithin.genus <-   chisq.test(table(Xdata_genus$Host_genus, Xdata_genus$any_coinf_within)))
(Xwithin.species <- chisq.test(table(Xdata_species$Host_species, Xdata_species$any_coinf_within)))

# Chi-squared tests - between-genus co-infection
(Xbetween.order <-   chisq.test(table(Xdata_order$Host_order, Xdata_order$any_coinf_between)))
(Xbetween.family <-  chisq.test(table(Xdata_family$Host_family, Xdata_family$any_coinf_between)))
(Xbetween.genus <-   chisq.test(table(Xdata_genus$Host_genus, Xdata_genus$any_coinf_between)))
(Xbetween.species <- chisq.test(table(Xdata_species$Host_species, Xdata_species$any_coinf_between)))
...
(cluster1 <- kmeans(any_coinf_order$p_coinf, centers = 3))
plot(any_coinf_order$p_coinf, col = cluster1$cluster)
