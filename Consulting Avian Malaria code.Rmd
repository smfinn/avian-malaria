...
posP.infspecies <- as.data.frame(table(malaria.posP$Host_species, malaria.posP$haemo_coinf))
coinf_species_infected <- data.frame(
  Host_species = posL.infspecies$Var1[co$Var2==0],
...
coinf_species_infected$coinf_Lwithin_infected <- coinf_species_infected$yes.Lw / 
  (coinf_species_infected$yes.Lw + coinf_species_infected$no.Lw)
coinf_species_infected$coinf_Pwithin_infected <- coinf_species_infected$yes.Pw / 
  (coinf_species_infected$yes.Pw + coinf_species_infected$no.Pw)
coinf_species_infected$coinf_Hwithin_infected <- coinf_species_infected$yes.Hw / 
  (coinf_species_infected$yes.Hw + coinf_species_infected$no.Hw)
coinf_species_infected <- coinf_species_infected[, c("Host_species",
   "coinf_Lwithin_infected", "coinf_Pwithin_infected", "coinf_Hwithin_infected")]

# Add co-infection rates among infected birds to main data frame
malaria <- merge(malaria, coinf_species_infected, by = "Host_species")
...
haemo_eda <- grid.arrange(haemo_eda1, haemo_eda2, nrow = 1, 
             top = "Haemocrodius Prevalence by Country")
...
haemo_eda_sex <- grid.arrange(haemo_eda3, haemo_eda4, nrow = 1, 
             top = "Haemocrodius Prevalence by Sex")
...
w.p.L <- mean(coinf_species_infected$coinf_Lwithin_infected, na.rm = T)
...
w.p.H <- mean(coinf_species_infected$coinf_Hwithin_infected, na.rm = T)
...
w.p.P <- mean(coinf_species_infected$coinf_Pwithin_infected, na.rm = T)
...
ggsave("frequency by genus1 weighted.png", wald_boxes_chart.w, height = 4.5, width = 5)
```
...
wald_boxes_chart2 <- ggplot(wald_boxes2, aes(genera, y)) +
  ...
  scale_color_manual(values = c("#1e7898", "#86b191", "#e49d60")) +
  theme(legend.position = "none") +
  labs(title = "Prevalnce of Malaria Infection by Genus (Unweighted)", 
...
wald_boxes_chart3 <- ggplot(wald_boxes3, aes(genera, y)) +
  ...
  scale_color_manual(values = c("#1e7898", "#86b191", "#e49d60")) +
  theme(legend.position = "none") +
  labs(title = "Prevalnce of Malaria Infection by Genus (Weighted)", 
       subtitle = "Among all birds screened for each genus", 
       x = "\n Genus", y = "Percent of birds infected")
wald_boxes_chart3

ggsave("frequency by genus3.png", wald_boxes_chart3, height = 4.5, width = 5)
```
...
wp3 <- mean(coinf_species$coinf_between_species, na.rm = T)
...
wci4.lower <- wp3 - qnorm(0.975) * sqrt((wp3 * (1 - wp3)) / screened_all)
wci4.upper <- wp3 + qnorm(0.975) * sqrt((wp3 * (1 - wp3)) / screened_all)
cat("Weighted 95% confidence interval: [", round(c(wci3.lower, wci3.upper)*100, 2), "] \n")
...
#### Logistic regression: Reduced taxonomic groups by counts; reference group is mode; add covariates (Year, Country, Elevation)
```{r cov.altreference.reduced.logreg}
# Any within-genus co-infection
Xdata_order_within$Country <- relevel(as.factor(Xdata_order_within$Country), "Kenya")
crrw.glm1 <- glm(any_coinf_within ~ Host_order + Year + Elevation + Country, data = Xdata_order_within, family = binomial)
summary(crrw.glm1)

Xdata_family_within$Country <- relevel(as.factor(Xdata_family_within$Country), "Kenya")
crrw.glm2 <- glm(any_coinf_within ~ Host_family + Year + Elevation + Country, data = Xdata_family_within, family = binomial)
summary(crrw.glm2)

Xdata_genus_within$Country <- relevel(as.factor(Xdata_genus_within$Country), "Kenya")
crrw.glm3 <- glm(any_coinf_within ~ Host_genus + Year + Elevation + Country, data = Xdata_genus_within, family = binomial)
summary(crrw.glm3)

Xdata_species_within$Country <- relevel(as.factor(Xdata_species_within$Country), "Kenya")
crrw.glm4 <- glm(any_coinf_within ~ Host_species + Year + Elevation + Country, data = Xdata_species_within, family = binomial)
summary(crrw.glm4)

# Any between-genera co-infection
Xdata_order_between$Country <- relevel(as.factor(Xdata_order_between$Country), "Kenya")
crrb.glm1 <- glm(any_coinf_between ~ Host_order + Year + Elevation + Country, data = Xdata_order_between, family = binomial)
summary(crrb.glm1)

Xdata_family_between$Country <- relevel(as.factor(Xdata_family_between$Country), "Kenya")
crrb.glm2 <- glm(any_coinf_between ~ Host_family + Year + Elevation + Country, data = Xdata_family_between, family = binomial)
summary(crrb.glm2)

Xdata_genus_between$Country <- relevel(as.factor(Xdata_genus_between$Country), "Kenya")
crrb.glm3 <- glm(any_coinf_between ~ Host_genus + Year + Elevation + Country, data = Xdata_genus_between, family = binomial)
summary(crrb.glm3)

Xdata_species_between$Country <- relevel(as.factor(Xdata_species_between$Country), "Kenya")
crrb.glm4 <- glm(any_coinf_between ~ Host_species + Year + Elevation + Country, data = Xdata_species_between, family = binomial)
summary(crrb.glm4)
```


```{r nested.logreg}
glm.full <- glm(any_coinf_within ~ Host_order + Host_family + Host_genus + 
                  Host_species + Year + Elevation + Country, 
                data = Xdata_species_within, family = binomial)
summary(glm.full)

nested1 <- glm(any_coinf_within ~ Host_order/Host_family, data = Xdata_order_within,
               family = binomial)
summary(nested1)

nested2 <- lmer(any_coinf_within ~ Year + Elevation + Country + 
                (1 | Host_order / Host_genus / Host_family / Host_species), data = Xdata_order_within)
summary(nested2)
```
