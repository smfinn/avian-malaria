...
country.table %>% gt(rowname_col = "Country") %>% 
...
gtsave(summary.x2.table, "Chi-Squared Summary Table.html")
```
...
X2.table.order.w %>% gt(rowname_col = "Order") %>% 
...
logreg.1 <- data.frame(Effects = c("Coliiformes", "Columbiformes", "Coraciiformes",
                       ...
                       lower.ci = exp(confint(crrw.glm1)[-1,1])*100,
                       upper.ci = exp(confint(crrw.glm1)[-1,2])*100,
...
logreg.tab1 <- logreg.1 %>% gt(rowname_col = "Effects", groupname_col = "effectcats") %>% 
  ...
  tab_header(title = md("**Within-Genus Co-Infection Estimates and Confidence Intervals for Avian Host Orders from Multivariate Logistic Regression**")) %>%
  ...
  cols_align(columns = c(Coefs, pvals, oddsratios, lower.ci, upper.ci), 
             align = "center") %>%
...
gtsave(logreg.tab1, "Within-Genus LogReg Order.html")
```
...
logreg.2 <- data.frame(Effects = c(sort(levels(Xdata_family_within$Host_family)[-1]),
                       ...
                       lower.ci = exp(confint(crrw.glm2)[-1,1])*100,
                       upper.ci = exp(confint(crrw.glm2)[-1,2])*100,
                       pvals = coef(summary(crrw.glm2))[-1, 4])

logreg.tab2 <- logreg.2 %>% gt(rowname_col = "Effects", groupname_col = "effectcats") %>% 
  tab_stubhead(label = md("**Effects**")) %>%
  tab_header(title = md("**Within-Genus Co-Infection Estimates and Confidence Intervals for Avian Host Families from Multivariate Logistic Regression**")) %>%
  fmt_number(columns = c(Coefs, oddsratios), decimals = 2) %>%
  fmt_number(columns = c(pvals), decimals = 3) %>%
  fmt_number(columns = c(lower.ci), decimals = 2, pattern = "[{x},") %>%
  fmt_number(columns = c(upper.ci), decimals = 2, pattern = "{x}]") %>%
...
gtsave(logreg.tab2, "Within-Genus LogReg Family.html")
```
...
logreg.3 <- data.frame(Effects = c(sort(levels(Xdata_genus_within$Host_genus)[-1]),
                       ...
                       lower.ci = exp(confint(crrw.glm3)[-1, 1])*100,
                       upper.ci = exp(confint(crrw.glm3)[-1, 2])*100,
                       pvals = coef(summary(crrw.glm3))[-1, 4])

logreg.tab3 <- logreg.3 %>% gt(rowname_col = "Effects", groupname_col = "effectcats") %>% 
  tab_stubhead(label = md("**Effects**")) %>%
  tab_header(title = md("**Within-Genus Co-Infection Estimates and Confidence Intervals for Avian Host Genera from Multivariate Logistic Regression**")) %>%
  fmt_number(columns = c(Coefs, oddsratios), decimals = 2) %>%
  fmt_number(columns = c(pvals), decimals = 3) %>%
  fmt_number(columns = c(lower.ci), decimals = 2, pattern = "[{x},") %>%
  fmt_number(columns = c(upper.ci), decimals = 2, pattern = "{x}]") %>%
...
gtsave(logreg.tab3, "Within-Genus LogReg Genus.html")
```
...
logreg.4 <- data.frame(Effects = c(sort(levels(Xdata_species_within$Host_species)[-1]),
                       ...
                       lower.ci = exp(confint(crrw.glm4)[-1, 1])*100,
                       upper.ci = exp(confint(crrw.glm4)[-1, 2])*100,
                       pvals = coef(summary(crrw.glm4))[-1, 4])

logreg.tab4 <- logreg.4 %>% gt(rowname_col = "Effects", groupname_col = "effectcats") %>% 
  tab_stubhead(label = md("**Effects**")) %>%
  tab_header(title = md("**Within-Genus Co-Infection Estimates and Confidence Intervals for Avian Host Species from Multivariate Logistic Regression**")) %>%
  fmt_number(columns = c(Coefs, oddsratios), decimals = 2) %>%
  fmt_number(columns = c(pvals), decimals = 3) %>%
  fmt_number(columns = c(lower.ci), decimals = 2, pattern = "[{x},") %>%
  fmt_number(columns = c(upper.ci), decimals = 2, pattern = "{x}]") %>%
...
gtsave(logreg.tab4, "Within-Genus LogReg Species")
```
...
logreg.5 <- data.frame(Effects = c(sort(levels(Xdata_order_between$Host_order)[-1]),
                       ...
                       lower.ci = exp(confint(crrb.glm1)[-1,1])*100,
                       upper.ci = exp(confint(crrb.glm1)[-1,2])*100,
                       pvals = coef(summary(crrb.glm1))[-1, 4])

logreg.tab5 <- logreg.5 %>% gt(rowname_col = "Effects", groupname_col = "effectcats") %>% 
  tab_stubhead(label = md("**Effects**")) %>%
  tab_header(title = md("**Between-Genus Co-Infection Estimates and Confidence Intervals for Avian Host Orders from Multivariate Logistic Regression**")) %>%
  fmt_number(columns = c(Coefs, oddsratios), decimals = 2) %>%
  fmt_number(columns = c(pvals), decimals = 3) %>%
  fmt_number(columns = c(lower.ci), decimals = 2, pattern = "[{x},") %>%
  fmt_number(columns = c(upper.ci), decimals = 2, pattern = "{x}]") %>%
...
gtsave(logreg.tab5, "Between-Genus LogReg Order.html")
```
...
logreg.6 <- data.frame(Effects = c(sort(levels(Xdata_family_between$Host_family)[-1]),
                       ...
                       lower.ci = exp(confint(crrb.glm2)[-1,1])*100,
                       upper.ci = exp(confint(crrb.glm2)[-1,2])*100,
                       pvals = coef(summary(crrb.glm2))[-1, 4])

logreg.tab6 <- logreg.6 %>% gt(rowname_col = "Effects", groupname_col = "effectcats") %>% 
  tab_stubhead(label = md("**Effects**")) %>%
  tab_header(title = md("**Between-Genus Co-Infection Estimates and Confidence Intervals for Avian Host Families from Multivariate Logistic Regression**")) %>%
  fmt_number(columns = c(Coefs, oddsratios), decimals = 2) %>%
  fmt_number(columns = c(pvals), decimals = 3) %>%
  fmt_number(columns = c(lower.ci), decimals = 2, pattern = "[{x},") %>%
  fmt_number(columns = c(upper.ci), decimals = 2, pattern = "{x}]") %>%
...
gtsave(logreg.tab6, "Between-Genus LogReg Family.html")
```

```{r logreg.table7}
logreg.7 <- data.frame(Effects = c(sort(levels(Xdata_genus_between$Host_genus)[-1]),
                                   "Year", "DRC", "Malawi", "Mozambique", "Uganda"),
                       effectcats = c(rep("Gebus (reference = Calandrella)", 11), 
                                      "Year", 
                                      rep("Country (reference = Kenya)", 4)),
                       Coefs = coef(summary(crrb.glm3))[-1, 1],
                       oddsratios = exp(coef(summary(crrb.glm3))[-1, 1]),
                       lower.ci = exp(confint(crrb.glm3)[-1, 1])*100,
                       upper.ci = exp(confint(crrb.glm3)[-1, 2])*100,
                       pvals = coef(summary(crrb.glm3))[-1, 4])

logreg.tab7 <- logreg.7 %>% gt(rowname_col = "Effects", groupname_col = "effectcats") %>% 
  tab_stubhead(label = md("**Effects**")) %>%
  tab_header(title = md("**Between-Genus Co-Infection Estimates and Confidence Intervals for Avian Host Genera from Multivariate Logistic Regression**")) %>%
  fmt_number(columns = c(Coefs, oddsratios), decimals = 2) %>%
  fmt_number(columns = c(pvals), decimals = 3) %>%
  fmt_number(columns = c(lower.ci), decimals = 2, pattern = "[{x},") %>%
  fmt_number(columns = c(upper.ci), decimals = 2, pattern = "{x}]") %>%
...
gtsave(logreg.tab7, "Between-Genus LogReg Genus.html")
```
...
anova(nested1, nested2)
anova(nested2, nested3)
anova(nested1, nested3)

plot(nested1, resid(., type = "pearson") ~ fitted(.) | any_coinf_within) ## concerning... need to look at weighting (https://slcladal.github.io/regression.html)
