...
summary.nested <- data.frame(
  order.variance = c(0.8685, 0.7291, 0.5452, 0.0000, 0.0000, 0.0000),
  family.variance = c(0.4303, 0.3470, 0.3164, 0.965, 0.182, 0.0936),
  genus.variance = c("-", 0.1993, 0.0629, "-", 1.079, 0.3567),
  ...
  AIC = c(round(aov1$"AIC"[1], 1), round(aov1$"AIC"[2], 1), round(aov2$"AIC"[1], 1),
          round(aov3$"AIC"[1], 1), round(aov3$"AIC"[2], 1), round(aov4$"AIC"[1], 1)),
...
summary.nested.table <- summary.nested %>% 
  ...
  tab_header(title = md("**Table 16.** Summary of ANOVA Results for Full and Reduced Generalized \nMixed Effect Regression Models")) %>%
